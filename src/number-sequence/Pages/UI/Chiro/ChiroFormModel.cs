...
namespace number_sequence.Pages.UI.Chiro
{
    ...
    public abstract class ChiroFormModel : PageModel
    {
        ...
        /// <summary>
        /// Leaves the form for the index page, telling it what was just recorded. The document name is worked out
        /// from the input rather than waited for: the pdf is generated by an orchestration that has not run yet, but
        /// its name is a pure function of the record, so it can be shown immediately.
        /// </summary>
        protected IActionResult RedirectToSubmitted(ChiroSpecies species, ChiroInput input, ChiroInputCreated created)
        {
            this.TempData[ChiroForm.SubmittedSpeciesKey] = species.ToString().ToLowerInvariant();
            this.TempData[ChiroForm.SubmittedRowIdKey] = created.RowId;
            this.TempData[ChiroForm.SubmittedDocumentKey] = ChiroDocumentNaming.AttachmentName(input);
            return this.Redirect("/ui/chiro");
        }
