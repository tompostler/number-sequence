...
namespace number_sequence.Controllers
{
    ...
    public sealed class CountsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateAsync([FromBody] Count count)
        {
            ...
            Count createdCount = await this.countDataAccess.CreateAsync(count);
        ...
        {
            Count count = await this.countDataAccess.TryGetAsync(this.User.Identity.Name, name);
        ...
        {
            Count count = await this.countDataAccess.IncrementAsync(this.User.Identity.Name, name);
        ...
        {
            Count count = await this.countDataAccess.IncrementByAmountAsync(this.User.Identity.Name, name, amount);
            return this.Request.Query.ContainsKey("bare")
                ? this.Ok(count.Value)
                : this.Ok(count);
        }
